...
class HomeScreen extends PureComponent {
  ...
  fetchAttendence() {
    runQuery(
      `SELECT * FROM time_tbl WHERE user_id = '${this.state.user_id}' ORDER BY id DESC`
    ).then(
      (result: any) => {
        result.forEach((r: any, ind: number) => {
          r.starttime = this.parseTime(
            r.start_time ? parseInt(r.start_time) : null
          );
  ...
  setTime = () => {
    ...
    runQuery(
      `SELECT * FROM time_tbl WHERE user_id='${this.state.user_id}' ORDER BY ID DESC LIMIT 1`
    ).then(
      (result: any) => {
        if (result.length == 0) {
          runQuery(
            `INSERT INTO time_tbl (user_id, start_time) VALUES ('${user_id}', '${time}')`
          );
        } else {
          ...
          if (result[0].start_time && !result[0].end_time) {
            runQuery(
              `UPDATE time_tbl SET user_id = '${user_id}', end_time = '${time}' WHERE id = ${result[0].id}`
            );
          }
        }
        runQuery(
          ...
          (result: any) => {
            result.forEach((r: any) => {
              ...
              r.endtime = this.parseTime(
                r.end_time ? parseInt(r.end_time) : null
              );
              r.total = "";
              if (r.start_time && r.end_time) {
                var diff = Math.abs(
                  parseInt(r.start_time) - parseInt(r.end_time)
                );
                // diff = 213854675;
                var remainder = diff % 3600000;
                var seconds = parseInt(
                  ((diff % (3600000 * 60)) / 1000).toFixed(0)
                );
    ...
    return (
      <View style={[globalstyles.container]}>
            ...
            >
              ...
              >
                {this.props.route.params.params.Name +
                  " " +
                  this.props.route.params.params.LastName}
              </Text>
        ...
        >
                ...
                return (
                  <View
                    style={{
                      ...
                      borderBottomWidth:
                        index == this.state.DATA.length - 1 ? 2 : 1,
                    }}
                  >
                    <Text
                      style={[
          ...
          <TouchableOpacity
            onPress={(index) => {
              this.state.isTimeStarted && this.trackChange(index);
              this.props["navigation"].navigate("Login");
            }}
          >
            <View
